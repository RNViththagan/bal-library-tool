...
public class DefaultValueGeneratorUtil {
    ...
    private static StringBuilder getDefaultValueForType(TypeSymbol bType, StringBuilder valueBuilder) {
        if (bType == null) {
            ...
            TypeSymbol rawType = CommonUtils.getRawType(bType);
            TypeDescKind typeKind = rawType.typeKind();
            switch (typeKind) {
                case TYPE_REFERENCE:
                    return getDefaultValueForType(CommonUtils.getRawType(bType), valueBuilder);
                case FLOAT:
                ...
                case RECORD:
                    ...
                    valueBuilder.append("{");
                    List<RecordFieldSymbol> mandatoryFieldSymbols = getMandatoryRecordFields(recordTypeSymbol)
                    ...
                    for (RecordFieldSymbol mandatoryField : mandatoryFieldSymbols) {
                        String fieldName = escapeReservedKeyword(mandatoryField.getName().get());
                ...
                case TUPLE:
                    ...
                    break;
                case ERROR:
                    TypeSymbol errorType = CommonUtils.getRawType(((ErrorTypeSymbol) rawType).detailTypeDescriptor());
                    StringBuilder errorString = new StringBuilder("error(\"\"");
                    if (errorType.typeKind() == TypeDescKind.RECORD) {
                        List<RecordFieldSymbol> mandatoryFields = getMandatoryRecordFields(
                                (RecordTypeSymbol) errorType);
                ...
                case UNION:
                    ...
                    boolean hasNilType = members.stream()
                            .anyMatch(mem -> CommonUtils.getRawType(mem).typeKind() == TypeDescKind.NIL);
                ...
                case INTERSECTION:
                    ...
                    effectiveType = CommonUtils.getRawType(effectiveType);
    ...
    private static List<RecordFieldSymbol> getMandatoryRecordFields(RecordTypeSymbol recordType) {
        return recordType.fieldDescriptors().values().stream()
                .filter(field -> !field.hasDefaultValue() && !field.isOptional())
                .toList();
    }

    private static String escapeReservedKeyword(String value) {
